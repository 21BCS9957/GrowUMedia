...
const steps = [
  ...
  },
  {
    icon: Target,
    title: "Custom Plan",
    description: "Get a tailored roadmap designed specifically to focus on business growth through YouTube.",
  ...
  },
  {
    icon: Zap,
    title: "Execution",
    description: "Our team handles everything while you focus on the business.",
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="hidden lg:block relative">
            {/* Animated Connection Line with Glass Effect */}
            <div className="absolute top-20 left-0 right-0 h-2 bg-gradient-to-r from-transparent via-white/20 to-transparent backdrop-blur-md border border-white/10 rounded-full">
              <div className="absolute inset-0 bg-gradient-to-r from-primary/40 to-secondary/40 animate-pulse opacity-70 rounded-full" />
                ...
                >
                  ...
                  <div className={`relative p-8 rounded-3xl border transition-all duration-500 ${activeStep === index
                    ...
                    <div className="absolute -top-4 left-8">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${activeStep === index
                    ...
                    <div className="relative z-10 space-y-6">
                      {/* Icon */}
                      <div className={`inline-flex w-16 h-16 rounded-2xl items-center justify-center transition-all duration-300 ${activeStep === index
                        ? `bg-gradient-to-br ${step.color} shadow-lg scale-110`
                        : 'bg-primary/10 group-hover:bg-primary/20'
                        }`}>
                        <step.icon className={`w-8 h-8 transition-colors duration-300 ${activeStep === index ? 'text-white' : 'text-primary'
                          }`} />
                      </div>

                      {/* Content */}
                      <div className="space-y-3">
                        <h3 className={`text-xl font-bold transition-colors duration-300 ${activeStep === index ? 'text-primary' : 'text-foreground'
                          }`}>
                          {step.title}
                        </h3>
                        ...
                        <div className={`transition-all duration-500 overflow-hidden ${activeStep === index ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'
                          }`}>
                      ...
                      {index < steps.length - 1 && (
                        <div className="absolute -right-4 top-1/2 -translate-y-1/2 hidden lg:block">
                          <ArrowRight className={`w-6 h-6 transition-all duration-300 ${activeStep === index ? 'text-primary scale-110' : 'text-muted-foreground'
                            }`} />
                        </div>
                      )}
                    </div>
